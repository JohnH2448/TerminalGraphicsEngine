...
With these tools, essentially any arrangement of data can be displayed so long as it can be compressed to a sufficiently low resolution. To display the canvas, simply run the script and it will render in the standard terminal used to call it. It is vital to ensure the terminal is broad enough to fit the entire frame. Terminals may auto indent when a line comes close to its boundry, and this will fracture the canvas. If the rendering looks broken, enlarge the terminal window to give comfortable room for the canvas to print properly. 
